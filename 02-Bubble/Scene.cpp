	...
	case Level1:
		...
		for (int i = 0; i < enemies.size(); i++) {
			...
			if ((current_x > cameraX - (SCREEN_WIDTH + (INIT_PLAYER_X_TILES + 1) * map->getTileSize()) && current_x < cameraX) && (current_y < cameraY && current_y > -16))
			{
				...
				if (current_x - player_x < 3 * map->getTileSize() && current_x - player_x > 0)
				{
					act = true;
				}
				...
				enemies[i]->update(deltaTime, act);
					...
					if (act && enemies[i]->getType() == 2) {
						if (enemies[i]->time_shot()) {
							int direccion;
							float posX, posY;
							...
							makeShot2(false, false, posX, posY, direccion);
						}
...
void Scene::makeShot2(bool playershot, bool vertical, float posX, float posY, int direccion)
{
	newShot();
	shots[shots.size() - 1] = new Shot();
